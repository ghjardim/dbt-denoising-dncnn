...
class denoiser(object):
    def __init__(self, sess, input_c_dim=3, batch_size=128):
        ...
        self.Y = dncnn(self.X, is_training=self.is_training, output_channels=1)
    ...
    def evaluate(self, iter_num, eval_files, noisy_files, summary_writer):
        ...
        for i in range(4):            
            ...
            clean_image = np.expand_dims(clean_image, 2)
...
class dataset(object):
  def __init__(self,sess):
    ...
    dataset = (
        ...
        .map(lambda item: tuple(tf.py_func(im_read, [item], [tf.float32,])))
...
def im_read(filename):
    ...
    image_decoded = np.load(filename.decode) 
    #image_string = tf.read_file(filename)
    #image_decoded = tf.image.decode_png(image_string, channels=1)    
    print(image_decoded)
    print("****")
    print("Shape do vetor de images")
    print(filename)
    ...
    return image_decoded.astype(np.float32)
...
def get_patches(image, num_patches=128, patch_size=64):
    ...
    print(patches.get_shape().dims)
    print([num_patches, patch_size, patch_size, 1])
